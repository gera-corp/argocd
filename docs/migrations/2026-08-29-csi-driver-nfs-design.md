...
**fsGroupPolicy.** Драйвер объявляет `fsGroupPolicy: File`, поэтому kubelet
меняет владельца тома при монтировании, если под задаёт `fsGroup`. In-tree
NFS-плагин так никогда не делал. При усыновлении openclaw это обошлось в разовый
рекурсивный chown 190 000 файлов и около 29 минут простоя на rpi5.

Плата единоразовая: чарт openclaw уже задаёт `fsGroupChangePolicy: OnRootMismatch`,
поэтому дальше kubelet сверяет только владельца корневого каталога. Проверено
рестартом после усыновления — под поднялся за 45 секунд. Тома без `fsGroup`
(seafile-data 16 ГБ, influxdb 14.5 ГБ) не затронуты вовсе.

На будущее: при усыновлении тома, у которого под задаёт `fsGroup` и много мелких
файлов, закладывать время на первый старт. Сменить `fsGroupPolicy` для драйвера
целиком нельзя без пересоздания объекта CSIDriver — поле иммутабельно.

**Значения helm-релизов loki и seafile.** В сохранённых values всё ещё
указан старый StorageClass `openmediavault-nfs-client`. StatefulSet'ы этих
релизов пересозданы точечно из их живых манифестов, а не через
`helm upgrade`, поэтому будущий `helm upgrade` любого из релизов вернёт
старый класс. Оба требуют отдельного осознанного прохода.
