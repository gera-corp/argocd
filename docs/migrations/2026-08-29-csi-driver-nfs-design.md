...
рекурсивно меняет владельца тома при каждом монтировании, если под задаёт
`fsGroup`. In-tree NFS-плагин так никогда не делал. Материально пострадал
только openclaw (190 000 файлов, ~29 минут на rpi5), и это исправлено точечно
на уровне пода через `fsGroupChangePolicy: OnRootMismatch`. Тома без `fsGroup`
— seafile-data (16 ГБ) и influxdb (14.5 ГБ) — не затронуты. Сменить
`fsGroupPolicy` для драйвера целиком нельзя без пересоздания объекта
CSIDriver: поле иммутабельно.
