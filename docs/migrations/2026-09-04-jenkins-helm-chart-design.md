...
Дата: 2026-09-04. Статус: **выполнено 2026-09-04**, план выполнения — отдельный
документ.

Кат прошёл без потери данных: 96 плагинов, обе джобы и история сборок на месте,
PVC `jenkins-pv` не пересоздавался. Проверено после переезда: вход по LDAP,
resolve всех credentials из Vault, сборка на агенте `jenkins-nomad-worker`.
Единственное изменение в поведении — плагины больше не обновляются сами при
рестарте пода, теперь это коммит в git.
